...
function SignIn({onLinkClickHandler}: Props) {         // Authentication에 선언된 onLinkClickHander를  sign-in에서 쓰기위해 Props 이용
  ...
  const signInResponse = (result: SignInResponseDto | ResponseDto | null) => {
    ...
    setCookie('accessToken', accessToken, { path: '/', expires: expiration })    // path 강제로 지정하는게 안전
  ...
  return (
    <div className="authentication-contents">
        <div className="authentication-input-container">
              ...
              onKeydownHandler={onPasswordKeydownHandler} message={message} error />
        </div>
...
function SignUp({onLinkClickHandler} : Props) {
  ...
  const onIdButtonClickHandler = () => {
      ...
      if(!id || !id.trim()) return;          // trim : 공백제거
  ...
  const onAuthNumberButtonClickHandler = () => {
      ...
      const requestBody: EmailAuthCheckRequestDto = {
        userEmail: email,
        // authNumber: authNumber          // 이름이 같으면 생략가능
        authNumber
      };
...
export default function Authentication() {
  ...
  //                    constant                    //
  const AuthenticationContents = page === 'sign-in' ? <SignIn onLinkClickHandler={onLinkClickHandler}/> : 
